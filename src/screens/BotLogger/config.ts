...
export const getQuestions = (): BotQuestion[] => {
  ...
  const [additionalQuestions, setAdditionalQuestions] = useState<BotQuestion[]>([])
            ...
            navigation.navigate('BotLoggerEmotions', {
              onDone: (emotions) => {
                ...
                emotions.forEach(emotion => {
                  setAdditionalQuestions((questions) => [...questions, getEmotionQuestion(emotion, tempLog)])
                })
  ...
  additionalQuestions.forEach(question => questions.push(question))

  questions.push({
    id: 'action_close',
    text: "Thanks for your time. See you later!",
    answers: [{
      type: 'button_primary',
      buttonText: "Close",
      action: ({ trigger }) => {
