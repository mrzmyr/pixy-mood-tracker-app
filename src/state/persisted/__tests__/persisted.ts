...
// oxlint-disable-next-line anti-slop/no-module-mocking -- state/persisted imports the Sentry SDK directly without an injection seam; the test asserts on captureException
...
describe("Storage", () => {
  ...
  it("should `load`", async () => {
    AsyncStorage.getItem = jest
      .fn()
      .mockReturnValueOnce(Promise.resolve('{"test": "test"}'));
  ...
  it("should throw when data is corrupt instead of returning null", async () => {
    AsyncStorage.getItem = jest
      .fn()
      .mockReturnValueOnce(Promise.resolve('{"items": [truncated'));
