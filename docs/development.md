...
- **Run files per worktree:** build locks, build logs, and Metro's PID and log live in `~/.cache/pixy-mood-tracker/checkouts/<hash>/`. `checkout.txt` names the worktree. Commands print the log path.
