...
- Folder layout: domain modules in `src/features/<name>/`, app-wide state (settings, analytics, persistence) in `src/state/<name>/`, generic hooks in `src/hooks/`. Keep a module's context, storage, hooks, and `__tests__/` together.
