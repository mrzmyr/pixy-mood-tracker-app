...
- Import files in `src` through `@/`. Log with `logger` from [`src/lib/logger.ts`](src/lib/logger.ts), never `console` or Sentry. Persist data with [`src/helpers/storage.ts`](src/helpers/storage.ts) and its `STORAGE_KEYS`, never AsyncStorage. Lint enforces all three.
